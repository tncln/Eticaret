    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult Register(Models.Account.RegisterModels user)
        {
            try
            {
                ...
                {
                    throw new Exception("Girdiğiniz Şifreler Aynı Değil");
                }
                if (context.Members.Any(x => x.Email == user.Member.Email))
                {
                    throw new Exception("Bu Eposta adresi Sistemde Kayıtlı");
                }
                if (user.Member.Password.Length < 6)
                {
                    throw new Exception("Şifre 7 karakterden küçük olamaz!!");
                }
        ...
        [HttpGet]
        public ActionResult Profil(int id = 0, int ad = 0)
        {
            ...
            if (id == 0)
            {
                ...
                if (ad != 0)
                {
                    currentAddress = context.Addresses.FirstOrDefault(x => x.Id == ad);
                }
            }
